									...
									$(this).iTip({
										...
										'icons' : [
											...
											},
											{
												...
												'click' : function()
												{
													document.location = 'manage.php?do=appsettings&update=1&id='+c+'&token=<?php echo $token; ?>';
												}
											}
										]
									})
...
?>
						<li class="listItem">
							<a href="manage.php?do=appsettings&delete=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Delete" class="secondaryContent delete"><span>Delete</span></a>
							<a href="manage.php?do=appsedit&id=<?php echo $row['id']; ?>" class="secondaryContent">Edit</a>
							<a href="<?php if ($settings) echo 'manage.php?do=appsettings&settings='.$row['folder'].'&token='.$token; else echo 'javascript:;'; ?>" class="secondaryContent <?php if (!$settings) echo "no-settings"; ?>">Settings</a>
							...
							<a href="manage.php?do=appsettings&activate=<?php echo $row['active']; ?>&id=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="<?php if ($row['active']==1) echo "Visible"; else echo "Hidden"; ?>" class="secondaryContent <?php if ($row['active']==1) echo "deactivate"; else echo "activate"; ?>"><span>Deactivate</span></a>
							<h4>
...
<?php
	...
	{
?>
									<a class="secondaryContent" href="manage.php?do=appsettings&f=<?php echo $folder['name']; ?>&token=<?php echo $token; ?>">Install</a>
<?php
					} 
					else 
					{
?>
									<a class="secondaryContent" href="javascript:void(0);">Install File Not Found</a>
<?php
					}
?>
								<h4>
...
?>
						<li class="listItem">
							<a href="manage.php?do=chatroomsettings&delete=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Delete" class="secondaryContent delete"><span>Delete</span></a>
				...
				<div class="module_content">
					...
					<fieldset class="firstFieldset">
								...
								<ul>
									<li>
										<label for="add_chatroom_img" style="position:relative">
											<input type="text" id="add_chatroom_img" class="selectionText" name="add_chatroom_img" value="<?php if(var_check('add_chatroom_img')) echo get_var('add_chatroom_img'); ?>" style="width: 200px" />
											<style>
												.chatroom-icon-select img{width:42px;height:42px;cursor:pointer}
											...
											<script type="text/javascript">
												$(document).ready(function() {
													...
													$(".chatroom-icon-select img").click(function() {
														$("#add_chatroom_img").val('essential-pack/'+$(this).attr('alt'));
													});
											...
											<?php
												$files = scandir(dirname(dirname(dirname(__FILE__))) . '/themes/' . $theme . '/images/icons/essential-pack/');
												...
												foreach ($files as $filename)
												{
													...
													{
											?>
												<div><img src="../themes/<?php echo $theme; ?>/images/icons/essential-pack/<?php echo $filename; ?>" alt="<?php echo $filename; ?>" /></div>
											<?php
													}
												}
											?>
										...
										or
										<input style="width:210px" type="file" name="add_chatroom_img_upload" />
									</li>
								</ul>
								<p class="explain">
								...
								<ul>
									<li>
										<label for="is_featured">
											<input type="checkbox" id="is_featured" name="is_featured" <?php if(!isset($is_featured)) echo 'checked="checked"'; else if($is_featured == 1) echo 'checked="checked"';  ?> value="1" />
											Feature Chat Room
										</label>
									</li>
								</ul>
								<p class="explain">
									Feature this chat room to make it display above other chat rooms.
								</p>
							</dd>
						</dl>
						<dl class="selectionBox">
						...
						<dl class="selectionBox">
							...
							<dt>
								<label for="add_chatroom_group"><?php if ($group_enable_mode == 1) echo 'Allowed'; else echo 'Disallowed'; ?> Group(s)</label>
							</dt>
							<dd>
								<select multiple="multiple" id="add_chatroom_group" name="add_chatroom_group[]" style="width: 452px; height: 125px;">
								...
								<p class="explain">
									Select the group(s) that should<?php if ($group_enable_mode != 1) echo ' not'; ?> have access to the room.  Hold Ctrl for multiple selections.
								</p>
							</dd>
							</div>
						</dl>
...
?>
					...
					<fieldset class="firstFieldset">
								...
								<label for="edit_chatroom_img" style="position:relative">
									...
									<script type="text/javascript">
										$(document).ready(function() {
											...
											$(".chatroom-icon-select img").click(function() {
												$("#edit_chatroom_img").val('essential-pack/'+$(this).attr('alt'));
											});
								...
								<ul>
									<li>
										<label for="edit_is_featured">
											<input type="checkbox" id="edit_is_featured" name="edit_is_featured" <?php if($row['is_featured'] == 1) echo 'checked="checked"';  ?> value="1" />
											Feature Chat Room
										</label>
									</li>
								</ul>
						...
						<dl class="selectionBox">
							...
							<div class="<?php if (ARROWCHAT_EDITION == "free") echo 'version-disabled'; ?>">
							<dt>
								<label for="edit_chatroom_group"><?php if ($group_enable_mode == 1) echo 'Allowed'; else echo 'Disallowed'; ?> Group(s)</label>
							</dt>
							<dd>
								<?php
									$disallowed_groups = @unserialize($row['disallowed_groups']);
									if (!is_array($disallowed_groups)) {
										$disallowed_groups = array();
									}
								?>
								<select multiple="multiple" id="edit_chatroom_group" name="edit_chatroom_group[]" style="width: 452px; height: 125px;">
...
?>
						...
						<dl class="selectionBox">
							<dt>
								<label for="remove_mod">Remove Mods/Admins</label>
							</dt>
							<dd>
								<select multiple="multiple" id="remove_mod" name="remove_mod[]" style="width: 452px; height: 125px;">
<?php
		$result = $db->execute("
			SELECT * 
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			{
				if (check_if_guest($row['user_id'])) {
					$mod_username = "Guest " . substr($row['user_id'], 1);
				} else {
					$result2 = $db->execute("
						SELECT " . DB_USERTABLE_NAME . " 
						FROM " . TABLE_PREFIX . DB_USERTABLE . " 
					...
					$row2 = $db->fetch_array($result2);
					$mod_username = $row2[DB_USERTABLE_NAME];
				}
?>
									<option value="<?php echo $row['user_id']; ?>"><?php echo $mod_username; ?></option>
<?php
			}
		} 
		else 
		{
?>
									<option value="0">There are no mods/admins in this chat room</option>
<?php
		}
?>
								</select>
								<p class="explain">
									Select the usernames that you would like to remove mod/admin status. Hold Ctrl to select multiple names. Do not use this for global mods/admins.
								</p>
							</dd>
						</dl>
					</fieldset>
...
<?php
		...
		if ($result AND $db->count_select() > 0 AND ARROWCHAT_EDITION != "free") 
		{
			while ($row = $db->fetch_array($result)) 
			{
?>
						<li class="listItem">
							<a href="manage.php?do=notificationsettings&delete=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Delete" class="secondaryContent delete"><span>Delete</span></a>
...
?>
						<li class="listItem">
							<a href="manage.php?do=traylinks&delete=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Delete" class="secondaryContent delete"><span>Delete</span></a>
							<a href="manage.php?do=traylinks&move=up&id=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Move Up" class="secondaryContent moveUp"><span style="<?php if($row['tray_location'] == '1') echo 'background:none !important;' ?>">Move Up</span></a>
							<a href="manage.php?do=traylinks&move=down&id=<?php echo $row['id']; ?>&token=<?php echo $token; ?>" title="Move Down" class="secondaryContent moveDown"><span style="<?php if($row['tray_location'] == $max_number) echo 'background: none !important;' ?>">Move Down</span></a>
...
?>
						...
						<dl class="selectionBox">
							...
							<dd>
								<ul>
									<li>
										<label for="link_icon">
											<input style="width: 200px" type="text" id="link_icon" class="selectionText" name="link_icon" value="<?php if(var_check('link_icon')) echo get_var('link_icon'); ?>" />
										</label>
										or
										<input style="width: 210px" type="file" name="link_icon_upload" />
									</li>
								</ul>
								<p class="explain">
			...
			<div class="title_bg"> 
				<div class="module_content">
					<div class="subtitle">Bar Link Settings</div>
