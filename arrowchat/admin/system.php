	...
	if (!empty($security_token)) 
	{
		if (hash_equals($_SESSION['token'], $security_token))
		{
			...
			if (var_check('config_submit')) 
			{
				...
				$result = $db->execute("
					...
					SET config_value = CASE 
						...
						WHEN config_name = 'spam_protection' THEN '" . trim(get_var('spam_protection')) . "'
						WHEN config_name = 'enable_jquery' THEN '" . trim(get_var('enable_jquery')) . "'
						WHEN config_name = 'enable_jquery_ui' THEN '" . trim(get_var('enable_jquery_ui')) . "'
					END WHERE config_name IN ('channel_name', 'base_url', 'login_url', 'online_timeout', 'disable_smilies', 'heart_beat', 'buddy_list_heart_beat', 'idle_time', 'push_on', 'push_ssl', 'push_publish', 'push_subscribe', 'spam_protection', 'enable_jquery', 'enable_jquery_ui')
				");
				...
				if ($result && $heart_beat_test) 
				{
					...
					$spam_protection = get_var('spam_protection');
					$enable_jquery = get_var('enable_jquery');
					$enable_jquery_ui = get_var('enable_jquery_ui');
			...
			if (var_check('maintenance_submit')) 
			{
				...
				header("Location: system.php?do=maintenance2&token=" . $token);
			}
			...
			if (var_check('repair_submit')) 
			{
				...
				$result = $db->execute("
					...
					VALUES  ('theme', 'new_facebook_full', 0), 
							...
							('push_encrypt', '".$push_random."', '0'), 
							('mobile_chat_icon', 'fas fa-comment', '0'), 
							('mobile_background_color', '0076ff', '0'), 
							('mobile_icon_color', 'ffffff', '0'), 
							('mobile_chat_action', '1', '0'), 
							('spam_protection', '0', '0'),
							('group_is_admin', '', '0'),
							('group_is_mod', '', '0'),
							('enable_rtl', '0', '0'),
