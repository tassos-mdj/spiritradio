(function(a){var b={init:function(b){var c=a.extend({closeButton:"true",direction:"right",marginVer:"30",marginHor:"5",icons:[{id:"thumbsup"}]},b);var d=a(this);var e=d.outerWidth();var f=d.outerHeight();var g=d.position().left;var h=d.position().top;var i=c.marginVer;var j=c.marginHor;var k="";a(c.icons).each(function(a,b){var c=b.id,tit="";if(c=="save")tit="Update"; else tit="Download";k+='<li class="icon" title='+tit+' id='+c+"></li>"});if(c.closeButton=="true"){if(c.direction=="left"){oldIconsHTML=k;k='<li class="icon close" id="close-left"></li>'+oldIconsHTML}if(c.direction=="right"||c.direction=="top"||c.direction=="bottom"){k+='<li class="icon close" id="close-right"></li>'}}var l="";switch(c.direction){case"right":l='<div class="triangle-border triangle-right-border"></div><div class="triangle triangle-right"></div>';break;case"left":l='<div class="triangle-border triangle-left-border"></div><div class="triangle triangle-left"></div>';break;case"bottom":l='<div class="triangle-border triangle-bottom-border"></div><div class="triangle triangle-bottom"></div>';break;case"top":l='<div class="triangle-border triangle-top-border"></div><div class="triangle triangle-top"></div>';break}d.after('<div class="itip-tooltip animated" id="test">'+l+'<ul class="icons">'+k+"</ul></div>");var m=d.next(".itip-tooltip");var n=m.outerWidth();var o=m.outerHeight();var p=m.find(".triangle").outerWidth();switch(c.direction){case"right":m.css("margin-top",h+f/2-o/2);m.css("margin-left",g+e+30);break;case"left":m.css("margin-top",h+f/2-o/2);m.css("margin-left",g-n-i);m.find(".triangle-left").css("left",n-3);m.find(".triangle-left-border").css("left",n-2);break;case"bottom":m.css("margin-top",h+f-j+o/2);m.css("margin-left",g+e/2-n/2);m.find(".triangle-bottom").css("left",n/2-p/2);m.find(".triangle-bottom-border").css("left",n/2-p/2);break;case"top":m.css("margin-top",h-j);m.css("margin-left",g+e/2-n/2);m.css("top","-50px");m.find(".triangle-top").css("left",n/2-p/2);m.find(".triangle-top-border").css("left",n/2-p/2);break}m.find(".icons").css("margin-top","-30px");var q="";var r="";if(typeof c.show=="undefined"){switch(c.direction){case"right":q="fadeInLeft";break;case"left":q="fadeInRight";break;case"bottom":q="fadeInUp";break;case"top":q="fadeInDown";break}}else{q=c.show}if(typeof c.close=="undefined"){switch(c.direction){case"right":r="fadeOutLeft";break;case"left":r="fadeOutRight";break;case"bottom":r="fadeOutUp";break;case"top":r="fadeOutDown";break}}else{r=c.close}if(Modernizr.cssanimations){m.addClass(q)}else{m.hide().fadeIn('fast')}if(c.closeButton=="true"){m.find(".close").on("click",function(){if(Modernizr.cssanimations){m.addClass(r)}else{m.fadeOut('fast')}if(jQuery.browser.webkit){m.on("webkitAnimationEnd",function(a){m.remove()})}if(jQuery.browser.mozilla){m.on("animationend",function(a){m.remove()})}if(jQuery.browser.opera){m.on("oAnimationEnd",function(a){m.remove()})}})}a(c.icons).each(function(a,b){if(typeof b.click=="function"){m.find("#"+b.id).on("click",function(){b.click.call(this)})}if(typeof b.hover=="function"){m.find("#"+b.id).on("mouseenter",function(){b.hover.call(this)})}})}};a.fn.iTip=function(c){if(b[c]){return b[c].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof c==="object"||!c){return b.init.apply(this,arguments)}else{a.error("Method "+c+" does not exist on jQuery.iTip")}}})(jQuery);
...
$(window).scroll(function() {
	if (0 == $(window).scrollTop()) {
		$("#subnavwrapper").removeClass("scrolled-down-header");
		$("#topnav").removeClass("scrolled-down-header");
	} else {
		$("#subnavwrapper").addClass("scrolled-down-header");
		$("#topnav").addClass("scrolled-down-header");
	}
});
if (0 == $(window).scrollTop()) {
	$("#subnavwrapper").removeClass("scrolled-down-header");
