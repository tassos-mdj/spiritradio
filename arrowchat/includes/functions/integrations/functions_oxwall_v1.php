<?php
	...
	*/
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . "base_avatar." . DB_USERTABLE_AVATAR . " avatar, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " link, arrowchat_status.is_admin, arrowchat_status.status 
			...
			LEFT JOIN " . TABLE_PREFIX . "base_avatar 
				ON  " . TABLE_PREFIX . "base_avatar .userId = " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " 
			...
			WHERE (" . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_USERID . " = '" . $db->escape_string($userid) . "' 
				...
				AND " . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_FRIENDS . " = 'active' 
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " link,  " . TABLE_PREFIX . "base_avatar." . DB_USERTABLE_AVATAR . " avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
			LEFT JOIN arrowchat_status 
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid 
			LEFT JOIN " . TABLE_PREFIX . "base_avatar 
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.jpg')) 
		{
			return $base_url . '../ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.jpg';
		} 
		else if (is_file(dirname(dirname(dirname(__FILE__))) . '/ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.gif')) 
		{
			return $base_url . '../ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.gif';
		} 
		else if (is_file(dirname(dirname(dirname(__FILE__))) . '/ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.png')) 
		{
			return $base_url . '../ow_userfiles/plugins/base/avatars/avatar_' . $user_id . '_' . $image . '.png';
		} 
		else 
		{
	...
	function get_group_id($userid)
	{
		...
		$result = $db->execute("
			SELECT roleId
			FROM " . TABLE_PREFIX . "base_authorization_user_role
	...
	function get_all_groups()
	{
		...
		$result = $db->execute("
			SELECT id, name
			FROM " . TABLE_PREFIX . "base_authorization_role
		");
