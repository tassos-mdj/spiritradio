<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		$result = $db->execute("
			SELECT e107_value
			FROM " . TABLE_PREFIX . "core
			WHERE e107_name = 'SitePrefs'
		");

		if ($row = $db->fetch_array($result))
		{
			$site_prefs = $row['e107_value'];
			eval("\$array = $site_prefs;");
			$cookie_name = $array['cookie_name'];
			
			if (isset($_COOKIE[$cookie_name]))
			{
				$cookie_value = $_COOKIE[$cookie_name];
				$userData = explode(".", $cookie_value);
				
				$result = $db->execute("
					SELECT user_password, user_id
					FROM " . TABLE_PREFIX . DB_USERTABLE . "
					WHERE user_id = '" . $db->escape_string($userData[0]) . "'
				");
				
				if ($row = $db->fetch_array($result))
				{
					if (md5($row['user_password']) == $userData[1])
					{
						$userid = $row['user_id'];
					}
				}
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../user.php?id.' . $link;
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if ($image) 
		{
			...
			$path = dirname(dirname(dirname(__FILE__))) . '/e107_media';
			...
			{
				return $base_url . '../e107_media/' . $folder . '/avatars/upload/' . $p[1];
			}
	...
	function get_all_groups()
	{
		...
		$result = $db->execute("
			SELECT userclass_id, userclass_name
			FROM " . TABLE_PREFIX . "userclass_classes
		");
		...
		{	 
			...
			{
				$groups[] = array($row['userclass_id'], $row['userclass_name']);
			}
