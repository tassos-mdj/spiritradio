<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE[$hash])) 
		{
			$result = $db->execute("
				...
				WHERE session_id='" . $db->escape_string($_COOKIE[$hash]) . "'
			");
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . "social_avatars.square avatar, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " link, arrowchat_status.is_admin, arrowchat_status.status 
			...
			LEFT JOIN " . TABLE_PREFIX . "social_avatars
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = " . TABLE_PREFIX . "social_avatars.uid
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " link,  " . TABLE_PREFIX . "social_avatars.square avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
			LEFT JOIN arrowchat_status 
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid 
			LEFT JOIN " . TABLE_PREFIX . "social_avatars
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../index.php/community/profile/' . $link;
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/media/com_easysocial/avatars/users/' . $user_id . '/' . $image)) 
		{
			return $base_url . '../media/com_easysocial/avatars/users/' . $user_id . '/' . $image;
		} 
		else 
		{
	...
	function get_group_id($userid)
	{
		...
		$result = $db->execute("
			SELECT group_id
			FROM " . TABLE_PREFIX . "user_usergroup_map
