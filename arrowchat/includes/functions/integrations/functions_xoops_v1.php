<?php
	...
	*/
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../modules/profile/userinfo.php?uid=' . $link;
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/uploads/' . $image) AND $image != "blank.gif") 
		{
	...
	function get_group_id($userid)
	{
		...
		$result = $db->execute("
			SELECT groupid
			FROM " . TABLE_PREFIX . "groups_users_link
	...
	function get_all_groups()
	{
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				$groups[] = array($row['groupid'], $row['name']);
			}
