<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (isset($_COOKIE['ccmAuthUserHash']))
		{
			$cookie = $_COOKIE['ccmAuthUserHash'];
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/application/files/avatars/' . $user_id . '.jpg')) 
		{
			return $base_url . '../application/files/avatars/' . $user_id . '.jpg';
		} 
		else 
		{
	...
	function get_group_id($userid)
	{
		...
		$result = $db->execute("
			...
			WHERE uID = '" . $db->escape_string($userid) . "'
		");
	...
	function get_all_groups()
	{
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				$groups[] = array($row['gID'], $row['gName']);
			}
