<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE['ac_data'])) {
            $userid = $_COOKIE['ac_data'];
        }

		if (!$userid && !empty($_COOKIE['ci_session'])) 
		{
			$result = $db->execute("
				SELECT user_data 
				FROM " . TABLE_PREFIX . "sessions
				WHERE ip_address = '" . $db->escape_string(ip2long($_SERVER['REMOTE_ADDR'])) . "'
			");
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../users/view/' . $link . '/profile';
	}
	...
	function get_avatar($image, $user_id) 
    {
        ...
        if (is_file(dirname(dirname(dirname(__FILE__))) . '/uploads/user-logo/' . (($user_id / 10000) % 10000) . '/'
                 . (($user_id / 1000) % 1000) . '/' . (($user_id / 100) % 100) . '/' . $user_id . '/middle-' . $image)) 
        {
            return $base_url . '../uploads/user-logo/' . (($user_id / 10000) % 10000) . '/'
                 . (($user_id / 1000) % 1000) . '/' . (($user_id / 100) % 100) . '/' . $user_id . '/middle-' . $image;
        } 
        else 
        {
