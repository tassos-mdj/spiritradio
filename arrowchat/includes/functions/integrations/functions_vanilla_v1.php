<?php
	...
	*/
	...
	function get_avatar($image, $user_id) 
	{
		...
		$avatar = explode($filename, $image);
		
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/uploads/' . $avatar[0] . '/n' . $filename)) 
		{
			return $base_url . '../uploads/' . $avatar[0] . '/n' . $filename;
		} 
		else 
		{
	...
	function get_all_groups()
	{
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				$groups[] = array($row['RoleID'], $row['Name']);
			}
