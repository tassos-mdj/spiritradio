<?php
	...
	*/
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../index.php/forum/profile/userid-' . $link;
	}
	...
	function get_group_id($userid)
	{
		...
		$result = $db->execute("
			SELECT rank
			FROM " . TABLE_PREFIX . "kunena_users
	...
	function get_all_groups()
	{
		...
		$result = $db->execute("
			SELECT rank_id, rank_title
			FROM " . TABLE_PREFIX . "kunena_ranks
		");
		...
		{	 
			...
			{
				$groups[] = array($row['rank_id'], $row['rank_title']);
			}
