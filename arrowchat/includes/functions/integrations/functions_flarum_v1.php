<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE['flarum_session'])) 
		{
			if (file_exists(dirname(dirname(dirname(__FILE__))) . '/storage/sessions/' . $_COOKIE['flarum_session']))
			{
				$file_data = file_get_contents(dirname(dirname(dirname(__FILE__))) . '/storage/sessions/' . $_COOKIE['flarum_session']);
				
				$session_data = @unserialize($file_data);
				
				if (!empty($session_data['user_id']))
				{
					$userid = $session_data['user_id'];
				}
				else if (!empty($session_data['access_token']))
				{
					$result = $db->execute("
						SELECT user_id 
						FROM " . TABLE_PREFIX . "access_tokens
						WHERE token = '" . $db->escape_string($session_data['access_token']) . "'
					");
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " link,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/assets/avatars/' . $image)) 
		{
			return $base_url . '../assets/avatars/' . $image;
		} 
		else 
		{
	...
	{	
		...
		{	 
			while ($row = $db->fetch_array($result))
			{
				...
				$groups[] = array($row['id'], $row['name_singular']);
			}
