<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE[$row['c'] . '_sid'])) 
		{
			$result = $db->execute("
				SELECT session_user_id 
				FROM " . TABLE_PREFIX . "sessions 
				WHERE session_id = '" . $db->escape_string($_COOKIE[$row['c'] . '_sid']) . "'
			");
	...
	function get_avatar($image, $user_id) 
	{
		...
		else if (is_file(dirname(dirname(dirname(__FILE__))) . DIRECTORY_SEPARATOR . 'images/avatars/gallery/' . $image)) 
		{
