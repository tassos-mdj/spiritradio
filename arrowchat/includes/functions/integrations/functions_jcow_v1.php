<?php
	...
	*/
	 ...
	 * all ONLINE friends that the user is friends with.  Also retrieve users that have been offline.
	 * for 60 seconds.  Otherwise, DO NOT retrieve offline users.  You can use global $online_timeout
	 * to get the online timeout.
	 * ex: AND (arrowchat_status.session_time + 60 + " . $online_timeout . ") > " . time() . " 
	 * Note: the + 60 demonstrates getting users that have been offline for up to 60 seconds.
	 *
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, " . TABLE_PREFIX . DB_USERTABLE . ".username link, arrowchat_status.is_admin, arrowchat_status.status 
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . ".username link,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
			LEFT JOIN arrowchat_status 
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid 
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/uploads/avatars/' . $image)) 
		{
