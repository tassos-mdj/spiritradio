<?php
	...
	*/
	...
	session_name($C->RNDKEY.str_replace(array('.','-'), '', $C->DOMAIN));
	session_start();
	...
	function get_user_id() 
	{
		...
		if (!empty($_SESSION['NETWORKS_USR_DATA']) AND !empty($_SESSION['NETWORKS_USR_DATA'][1]) AND !empty($_SESSION['NETWORKS_USR_DATA'][1]['LOGGED_USER']) AND !empty($_SESSION['NETWORKS_USR_DATA'][1]['LOGGED_USER']->id))
		{
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/storage/avatars/' . $image)) 
		{
