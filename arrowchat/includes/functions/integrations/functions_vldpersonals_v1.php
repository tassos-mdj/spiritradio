<?php
	...
	*/
	...
	function get_friend_list($userid, $time)
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity,
				concat(SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-1,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-2,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-3,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-4,1),'/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . ",'/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . ") avatar,
				" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " link, arrowchat_status.is_admin, arrowchat_status.status
			FROM " . TABLE_PREFIX . DB_FRIENDSTABLE . "
			JOIN " . TABLE_PREFIX . DB_USERTABLE . "
				ON (
					" . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_FRIENDID . " = " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " AND " . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_USERID . " = '" . $db->escape_string($userid) . "' OR
					" . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_USERID . " = " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " AND " . TABLE_PREFIX . DB_FRIENDSTABLE . "." . DB_FRIENDSTABLE_FRIENDID . " = '" . $db->escape_string($userid) . "'
					)
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " link,
				concat(SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-1,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-2,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-3,1),'/',SUBSTRING(" . TABLE_PREFIX . DB_USERTABLE . ".joindate,-4,1),'/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . ",'/'," . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . ") avatar,
				arrowchat_status.is_admin, arrowchat_status.status
			FROM " . TABLE_PREFIX . DB_USERTABLE . "
			LEFT JOIN arrowchat_status
				ON " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = arrowchat_status.userid
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../?m=member_profile&id=' . $link;
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (count($image) == 6) 
		{
			$image = $image[0] . '/' . $image[1] . '/' . $image[2] . '/' . $image[3] . '/' . $image[4] . '/t_photo_' . $image[5];
		}
		else 
		{
			$image = implode('/', $image);
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/media/uploads/' . $image)) 
		{
			return $base_url . '../media/uploads/' . $image;
		} 
		else 
		{
	...
	function get_all_groups()
	{
		...
		$result = $db->execute("
			...
			FROM " . TABLE_PREFIX . "members_groups
		");
