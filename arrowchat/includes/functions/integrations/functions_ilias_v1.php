	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE['PHPSESSID'])) 
		{
			$result = $db->execute("
				SELECT user_id
				FROM " . TABLE_PREFIX . "usr_session
				WHERE session_id = '" . $db->escape_string($_COOKIE['PHPSESSID']) . "'
			");
	...
	function get_friend_list($userid, $time)
	{
		...
		$sql = ("
			SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, concat(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . ", ' ', " . TABLE_PREFIX . DB_USERTABLE . ".lastname) username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " link, arrowchat_status.is_admin, arrowchat_status.status 
	...
	function get_user_details($userid) 
	{
		...
		$sql = ("
			SELECT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, concat(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . ", ' ', " . TABLE_PREFIX . DB_USERTABLE . ".lastname) username, arrowchat_status.session_time lastactivity,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " link,  " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, arrowchat_status.is_admin, arrowchat_status.status 
			FROM " . TABLE_PREFIX . DB_USERTABLE . " 
	...
	function get_link($link, $user_id) 
	{
		...
		return $base_url . '../ilias.php?col_side=right&block_type=pdusers&user=' . $link . '&cmd=showUserProfile&cmdClass=ilusersonlineblockgui&cmdNode=bc:7a:9b&baseClass=ilPersonalDesktopGUI';
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		$path = dirname(dirname(dirname(__FILE__))) . '/data';
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/data/' . $folder . '/usr_images/usr_' . $user_id . '_xxsmall.jpg')) 
		{
			return $base_url . '../data/' . $folder . '/usr_images/usr_' . $user_id . '_xxsmall.jpg';
		} 
		else 
		{
