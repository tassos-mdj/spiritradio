	...
	*/
	...
	function get_user_id() 
	{
		...
		{ 
			$result = $db->execute("
				...
				WHERE sid = '" . $db->escape_string($_COOKIE['sid']) . "'
			");
	...
	function get_friend_list($userid, $time) 
	{
		...
		$sql = ("
			...
			WHERE  FIND_IN_SET(" . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . ", (SELECT buddylist FROM " . TABLE_PREFIX . DB_USERTABLE . " WHERE " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " = " . $db->escape_string($userid) . "))
				AND !FIND_IN_SET(" . $db->escape_string($userid) . ", " . TABLE_PREFIX . DB_USERTABLE . ".ignorelist)
	...
	function get_group_id($userid)
	{
		...
		{	 
			if ($row = $db->fetch_array($result))
			{
				$group_ids[] = $row['usergroup'];
				
				$tmp = explode(",", $row['additionalgroups']);
	...
	function get_all_groups()
	{
		...
		{	 
			...
			{
				$groups[] = array($row['gid'], $row['title']);
			}
