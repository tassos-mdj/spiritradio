<?php
	...
	*/
	...
	if (ini_get('session.save_handler') == 'files') {
		 ini_set('session.save_path', dirname(dirname(dirname(__FILE__))) . '/files/tmp/');
	}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/files/avatars/' . $user_id . '.jpg')) 
		{
			return $base_url . '../files/avatars/' . $user_id . '.jpg';
		} 
		else 
		{
