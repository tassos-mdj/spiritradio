<?php
	...
	*/
	...
	function get_user_id() 
	{
		...
		if (!empty($_COOKIE['dle_user_id'])) 
		{
			$result = $db->execute("
				SELECT password, user_id
				FROM " . TABLE_PREFIX . DB_USERTABLE . "
				WHERE user_id = '" . $db->escape_string($_COOKIE['dle_user_id']) . "'
			");

			if ($row = $db->fetch_array($result))
			{
				if ($row['password'] == md5($_COOKIE['dle_password']))
				{
					$userid = $row['user_id'];
				}
			}
	...
	function get_avatar($image, $user_id) 
	{
		...
		if (is_file(dirname(dirname(dirname(__FILE__))) . '/uploads/fotos/' . $image)) 
		{
			return $base_url . '../uploads/fotos/' . $image;
		} 
		else 
		{
	...
	function get_all_groups()
	{
		...
		{	 
			...
			{
				$groups[] = array($row['id'], $row['group_name']);
			}
