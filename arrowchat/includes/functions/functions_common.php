<?php
	...
	*/
	function create_guest_hash()
	{
		...
		$result = $db->execute("
			INSERT INTO arrowchat_status (userid, hash_id, session_time) 
			VALUES ('" . $db->escape_string($userid) . "', '" . $hash_id . "', '" . time() . "')
		");
	...
	*/
	...
	/**
	 * Returns the most recent message from the user
	 *
	 * @param	string	$userid	The user to retrieve the message for
	 * @return the last message from the user
	 */
	function get_recent_message($retrieval_user_id)
	{
		global $db;
		global $language;
		global $userid;
		...
		{
			if (array_key_exists($retrieval_user_id, $clear_chats))
			{
				$time_check = $clear_chats[$retrieval_user_id];
			} 
			else 
			{
				$time_check = 0;
		...
		$result = $db->execute("
			SELECT arrowchat.message
			FROM arrowchat
			WHERE (arrowchat.from = '" . $db->escape_string($userid) . "' AND arrowchat.to = '" . $db->escape_string($retrieval_user_id) . "'
				OR arrowchat.to = '" . $db->escape_string($userid) . "' AND arrowchat.from = '" . $db->escape_string($retrieval_user_id) . "')
		...
		if ($result AND $db->count_select() > 0)
		{
			...
			if (substr($message, 0, 4) == "<div" OR substr($message, 0, 5) == "<span")
			{
				$message = $language[246];
			}
	...
	*/
	function updateUserSession() 
	{
		...
		{
			if ($session_time + ($online_timeout * 2) < $time)
			{
				$db->execute("
					UPDATE arrowchat_status 
					SET session_time = " . $time . ", session_start_time = " . $time . ", popout = " . $time . "
					WHERE userid = '" . $db->escape_string($userid) . "'
				");
		...
		else 
		{
			if ($session_time + ($online_timeout * 2) < $time)
			{
				$db->execute("
					UPDATE arrowchat_status 
					SET session_time = " . $time . ", session_start_time = " . $time . "
					WHERE userid = '" . $db->escape_string($userid) . "'
				");
	...
	*/
	function clickable_links($str, $popup = TRUE) 
	{
	...
	*/
	...
	/**
	 * Converts CSS into RTL
	 *
	 * @param array	The css
	 * @return	string	The full CSS in a string
	*/
	function rtl_callback($match)
	{
		$match[0] = preg_replace('/padding:\s*?(\d*p?x?)\s*(\d*p?x?)\s*(\d*p?x?)\s*(\d*p?x?)\s*?/i', 'padding: $1 $4 $3 $2', $match[0]);
		$match[0] = preg_replace('/margin:\s*?(\d*p?x?)\s*(\d*p?x?)\s*(\d*p?x?)\s*(\d*p?x?)\s*?/i', 'margin: $1 $4 $3 $2', $match[0]);
		$match[0] = preg_replace('/direction:\s*?(ltr)\s*?/i', 'direction:rtl', $match[0]);
		$match[0] = preg_replace('/flex-direction:\s*?(row)\s*?/i', 'flex-direction:row-reverse', $match[0]);
		$match[0] = strtr($match[0], array('left' => 'right', 'right' => 'left'));
		$match[0] = strtr($match[0], array('flex-start' => 'flex-end', 'flex-end' => 'flex-start'));
		return $match[0];
	}
