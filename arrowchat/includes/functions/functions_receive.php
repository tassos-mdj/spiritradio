		...
		foreach ($chatroomid as $key => $value)
		{
			...
			while ($row = $db->fetch_array($result)) 
			{
				if (!in_array($row['id'], $_SESSION['chatroom_mess_ids'])) 
				{
					...
					$chatroom[] = array('id' => $row['id'], 'userid' => $row['user_id'], 'n' => $row['username'], 'm' => $chat_message, 't' => $row['sent'], 'global' => $row['global_message'], 'mod' => $row['is_mod'], 'admin' => $row['is_admin'], "chatroomid" => $value, 'action' => $row['action']);
