		...
		while ($chat = $db->fetch_array($result)) 
		{
			if ($chat['userid'] != $userid) 
			{
				...
				if (!in_array($chat['userid'], $block_chats_unserialized))
				{
					...
					{
						$buddyList[] = array('id' => $chat['userid'], 'n' => strip_tags($chat['username']), 's' => $chat['status'], 'a' => $avatar, 'l' => $link, 'admin' => $chat['is_admin']);
					}
				}
			}
		}
			...
			while ($chat = $db->fetch_array($result)) 
			{
				if ($chat['userid'] != $userid) 
				{
					...
					if (!in_array($chat['userid'], $block_chats_unserialized))
					{
						if (!empty($chat['username']) AND $show_user)
						{
							$buddyList[] = array('id' => $chat['userid'], 'n' => stripslashes(strip_tags($chat['username'])), 's' => $chat['status'], 'a' => $avatar, 'l' => $link, 'admin' => $chat['is_admin']);
						}
					}
				}
			}
