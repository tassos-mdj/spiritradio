<?php
	...
	*/
	...
	if (logged_in($userid)) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			if ((empty($row['ban_length']) OR ((($row['ban_length'] * 60) + $row['ban_time']) > time())) AND $is_admin != 1) 
			{
				...
				{
					$error[] = array('t' => '3', 'm' => $language[55] . $chatroom_name);
				}
				else
				{
					$error[] = array('t' => '3', 'm' => $language[56] . $chatroom_name . $language[219] . $row['ban_length'] . $language[220]);
				}
	...
	{
		$result = $db->execute("
			SELECT type, name, welcome_message, image, description, password, author_id, max_users, limit_message_num, limit_seconds_num, disallowed_groups
			FROM arrowchat_chatroom_rooms 
			WHERE id = '" . $db->escape_string($chatroom_id) . "'
		");
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$roominfo[] = array('welcome_msg' => $row['welcome_message'], 'image' => $row['image'], 'desc' => $row['description'], 'limit_msg' => $row['limit_message_num'], 'limit_sec' => $row['limit_seconds_num']);
	...
	if (logged_in($userid)) 
	{
		...
		else if (!empty($disallowed_groups))
		{
			if (check_array_for_match($group_id, $disallowed_groups))
			{
				$error[] = array('t' => '1', 'm' => $language[210]);
	...
	if (logged_in($userid)) 
	{
		...
		if (empty($row['author_id'])) $row['author_id'] = "";
		...
		$db->execute("
			...
			VALUES (
				...
				'" . $db->escape_string($chatroom_invisible) . "',
				'" . $time . "') 
			ON DUPLICATE KEY 
		...
		if ($i > $chatroom_max_users AND $is_admin != 1 AND $is_mod != 1 AND $chatroom_max_users != 0)
		{
			$db->execute("
				...
				SET session_time = '0'
				WHERE chatroom_id = '" . $db->escape_string($chatroom_id)."'
					AND user_id = '" . $db->escape_string($userid)."'
			");
	...
	{	
		...
		foreach ($temp_array as $chatroom_history)
		{
			...
			$chat_history[] = array('id' => $chatroom_history['id'], 'n' => strip_tags($chatroom_history['username']), 'm' => $chat_message, 't' => $chatroom_history['sent'], 'a' => $avatar, 'userid' => $fetchid, 'global' => $chatroom_history['global_message'], 'mod' => $chatroom_history['is_mod'], 'admin' => $chatroom_history['is_admin']);
		}
