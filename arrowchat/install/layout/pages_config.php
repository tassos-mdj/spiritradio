						...
						$(document).ready(function() {
							...
							$("#all_online").click(function() {
								if(!$('#guests_only').is(':checked')) $(".friends_table").css("display", "table-row");
							});
							$("#friends_only").click(function() {
								if(!$('#guests_only').is(':checked')) $(".friends_table").css("display", "table-row");
							});
							$("#guests_only").click(function() {
								$(".standalone-info").css("display", "none");
								$(".database-prefix").css("display", "none");
							});
							$("#guests_chat").click(function() {
								$(".standalone-info").css("display", "table-row");
								$(".database-prefix").css("display", "table-row");
								if($('#no_friend_system').is(':checked')) $(".friends_table").css("display", "none");
							});
		...
		<?php
			...
			else
			{
				...
				if ($result AND $db->count_select() > 0)
				{
					...
					if ($prefix_guess == "arrowchat_" OR $prefix_guess == "searchtowords_" OR $prefix_guess == "sys_" OR $prefix_guess == "video_" OR $prefix_guess == "core_" OR $prefix_guess == "skin_" OR $prefix_guess == "blog_" OR $prefix_guess == "cms_" OR $prefix_guess == "posts_" OR $_SESSION['version'] == "ilias_v1")
					{
						$prefix_guess = "";
					}
				}
			...
			?>
				<form method="post" id="config_form" action="<?php echo $_SERVER['PHP_SELF']; ?>?mode=config">
					<p class="subtext">Enter the configuration details for your ArrowChat installation below.  Make sure these details are correct or your installation will not work properly.</p>
					<p>&nbsp;</p>
					<p>&nbsp;</p>
					...
					<div class="buddy_list_setup">
						<ul>
							...
							<li class="int_button_li2" style="text-align:left;width:719px;height:110px"><label class="int_button_label2" style="width:717px;height:92px"><input disabled="disabled" type="radio" class="step2_input" name="who_chat" id="guests_only" value="guests_only" /><img src="./images/guests_only.png" alt="Guests Only" style="float: left;position:relative;top:-5px;left:15px;width:90px;height:90px" /><div class="inside" style="float: left;margin-left:50px;margin-top:0px;"><b>Guests Only</b> (No additional programming required on custom sites)<span style="padding-left:0;width:550px;">This option will install ArrowChat using guests only and will not integrate with the users on your site.  Everyone using ArrowChat will be considered a guest.</span></div></label></li>
						</ul>
					</div>
						...
						<ul>
							...
							<li class="int_button_li3" style="width:463px;">
								<div style="float: right; padding-top:17px; width: 250px; text-align: left; line-height:2.3em;">
									...
									1. Create an account <a href="http://www.arrowchat.com/push/" target="_blank">here</a><br />
									2. Wait for the confirmation email<br />
									3. Enter your new API keys:<br />
					...
					<div class="step">
						<img src="./images/step4.png" alt="" /><strong> Complete your configuration details</strong>
					</div>
					
					<table class="form-table2" style="margin-left: 30px"> 
						<tr> 
							<th scope="row"><label for="phpver">ArrowChat Path</label></th> 
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="config_path" value="<?php echo $config_path; ?>" /></span></td> 
						...
						<tr class="no-border database-prefix"> 
							<th scope="row"><label for="configwrite">Database Prefix</label></th> 
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="prefix" value="<?php echo $prefix_val; ?>" /></span></td> 
						...
						<tr class="standalone-info">
							<td colspan="3" style="background-color:#fff;"><p class="subtext"><b style="font:inherit;color:#333;font-weight:bold">Database Details</b><br />In order for ArrowChat to connect with your existing database, we need some details about your database.  Consult our <a style="font:inherit" href="https://www.arrowchat.com/documentation/?p=custom-website" target="_blank">custom installation guide</a> if you need additional assistance with this.</p></td>
						</tr>
						<tr class="standalone-info"> 
							<th scope="row"><label for="configwrite">User Table</label></th> 
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="config_table_user" value="<?php echo $config_table_user; ?>" /></span></td> 
						...
						<tr class="standalone-info"> 
							...
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="config_field_username" value="<?php echo $config_field_username; ?>" /></span></td> 
						...
						<tr class="standalone-info"> 
							...
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="config_field_userid" value="<?php echo $config_field_userid; ?>" /></span></td> 
						...
						<tr class="standalone-info"> 
							...
							<td class="col2"><span class="formwrap"><input class="database_input" disabled="disabled" type="text" name="config_field_avatar" value="<?php echo $config_field_avatar; ?>" /></span></td> 
